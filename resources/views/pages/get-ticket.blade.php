...
<body class="p-1 h-screen">
    <div class=" h-full w-full p-3 flex flex-row gap-3" id="app">
        {{-- display info --}}
        <div class="bg-white border rounded-xl p-3 basis-full">
            <p id="errorMsg" class="hidden" style="color:#ef4444;margin-top:1rem;font-size:0.9rem;"></p>
        </div>
        {{-- the controls --}}
        <div class="bg-white border rounded-xl p-3 flex flex-col gap-3 basis-full">
            <div class="ticket-label text-center">Select Queue Type</div>
            {{-- Type Selection --}}
            <div id="selectionArea" class="basis-full">
                <div class="grid grid-cols-3 grid-rows-3 gap-3 h-full">
                    @foreach ($types as $type)
                        <button class="type-btn" data-code="{{ $type->code }}"
                            onclick="getTicket('{{ $type->code }}')">
            ...
            {{-- Ticket Result --}}
            <div class="selected-ticket p-3 flex justify-center text-center h-full" id="ticketResult">
                <div class="">
                    <div class="label">Your Queue Number</div>
                    <div class="value" id="ticketValue">A-001</div>
                    <button class="btn-print" onclick="window.print()">Print</button>
                    <button class="btn-print" style="background:#6b7280;margin-left:0.5rem;" onclick="resetPage()">Get
                        Another</button>
                </div>
            </div>
        </div>
